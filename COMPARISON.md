...
### 2-5. 같은 use case의 코드 차이 (대표 snippet)

표만으로는 잘 안 와닿으니 **같은 기능(`save_reflection`)이 양쪽에서 어떻게 다르게 생겼는지** 실제 코드를 본다.

#### `_ensure_init` — 거의 같음, command만 다름

양쪽 모두 subprocess + asyncio loop + 도구 dict 캐시. **단 한 줄만 다르다**:

```python
# 외부 MCP — confluence_official.py
...
# 직접 구현 — confluence_custom.py
client = MultiServerMCPClient({
    "confluence_custom": {
        "command": sys.executable,                            # ← 같은 Python
...
**나머지 init 로직(threading.local, asyncio.new_event_loop, get_tools)은 완전히 동일**. → 2-4-1 공통 보일러플레이트의 실증.

---

#### `save_reflection` — 양쪽이 결정적으로 다른 부분

##### ▶ 외부 MCP — 클라이언트 wrapper가 모든 일을 함 (~60줄)

```python
# confluence_mcp/confluence_official.py
def save_reflection(agent_name, day, text, quota,
                    run_id=None, mode="append") -> bool:
    tools = _ensure_init()

    # ① 도메인 → Confluence 번역: 제목 prefix 조립
    title = _build_reflection_title(agent_name, day, run_id, mode)

    # ② 도메인 → Confluence 번역: 본문 markdown 조립
    ...
    # ③ mode 분기: overwrite면 find → update, 아니면 fallthrough → create
    if mode == "overwrite":
    ...
    # ④ create_page 호출 — space_key, parent_id 등 인프라 인자 매번 명시
...
> **여기서 보이는 것**: 도메인 변환(①②), mode 분기(③), 인프라 인자 명시(④) 모두 **클라이언트가 직접**. mcp-atlassian은 일반 CRUD만 안다.

##### ▶ 직접 구현 — 책임이 3계층으로 분산

**[1] 클라이언트 wrapper — 거의 비어있음 (~15줄)**

```python
# confluence_mcp/confluence_custom.py
def save_reflection(agent_name, day, text, quota,
                    run_id=None, mode="append") -> bool:
    tools = _ensure_init()
    ...
    _local.loop.run_until_complete(save_tool.ainvoke({
        ...
        "run_id": run_id, "mode": mode,                # ← 도메인 인자만 그대로 전달
    }))
    return True
```

> 도메인 인자를 받아서 그대로 MCP 도구에 전달. 변환·분기 0줄.

**[2] 서버의 도메인 도구 — 변환을 여기서 (~5줄)**

```python
# confluence_mcp_server/server.py
@mcp.tool()
def save_reflection(agent_name, day, text, quota=None,
                    run_id=None, mode="append") -> dict:
    title = _build_reflection_title(agent_name, day, run_id, mode)   # ① 도메인 변환
    html  = _build_html_body(agent_name, day, text, quota)           # ② 도메인 변환
    return _upsert(SPACE_KEY, title, html, PARENT_PAGE_ID, mode)     # ③ 분기 위임
```

> 외부 MCP의 ①②와 같은 일을 하지만 **서버 안**. 클라이언트엔 안 보임.
...
def _upsert(space_key, title, html_body, parent_id, mode) -> dict:
    if mode == "overwrite":
        existing = find_page_by_title(space_key, title)              # REST 호출 1
        if existing:
            return update_page(
                page_id=existing["id"], title=title,
                html_body=html_body,
                current_version=existing["version"],                  # ← version+1 우리가 처리
            )                                                         # REST 호출 2
    return create_page(
...
> 외부 MCP는 wrapper에 있던 ③ 분기가 **여기 한 곳**으로 옮겨옴. `version+1` 같은 Atlassian REST 디테일은 `update_page`(아래) 안에서.

**[4] REST 호출 — Atlassian 디테일 우리가 직접 (외부 MCP는 안 보이는 부분)**

`_upsert`가 부르는 REST 함수 3개. 모두 `confluence_mcp_server/confluence_client.py`.

```python
# 검색 — overwrite 모드에서 "이미 같은 제목 있나?" 확인
def find_page_by_title(space_key, title) -> dict | None:
    raw = _request("GET", "content", params={
        "spaceKey": space_key, "title": title,
        "expand": "version",                                          # ← version 정보 같이 받기 (update에 필요)
    })
...
# 생성 — append 또는 overwrite-못찾음에서 호출
...
> **주목할 디테일**: `update_page`의 `version+1`. Confluence는 동시 수정 충돌을 막기 위해 낙관적 락을 쓰는데, 클라이언트가 현재 version 받아서 +1해 보내야 함. **mcp-atlassian이 알아서 처리해주던 부분을 직접 구현은 우리가 책임**. `find_page_by_title`이 `expand=version`을 미리 받아두는 이유도 이 +1을 위해서.

---

#### 한눈에 — 같은 일을 하는 코드 위치

| 일 | 외부 MCP 위치 | 직접 구현 위치 |
|---|---|---|
| 제목 prefix 조립 | wrapper `save_reflection` | 서버 `save_reflection` (`_build_reflection_title`) |
| 본문 markdown/HTML 조립 | wrapper `save_reflection` | 서버 `save_reflection` (`_build_html_body`) |
| `mode` 분기 (find→update/create) | wrapper `save_reflection` | 서버 `_upsert` |
| 인프라 인자 (space_key, parent_id) | wrapper가 매번 명시 | 서버 상수 (`SPACE_KEY`, `PARENT_PAGE_ID`) |
| Atlassian `version+1` | mcp-atlassian이 처리 (안 보임) | 우리 `confluence_client.update_page` |

**클라이언트(호출 측) 코드량 비교**:
- 외부 MCP `save_reflection`: 약 60줄
- 직접 구현 `save_reflection` wrapper: 약 15줄
- 직접 구현 서버 측 (`@mcp.tool` + `_upsert` + `update_page`): 약 30줄

→ 클라이언트만 보면 직접 구현이 압도적으로 짧지만, **총합은 비슷하거나 직접 구현이 더 많다** (서버 보일러플레이트 포함). 1차에서 다룬 trade-off의 코드 레벨 실증.
